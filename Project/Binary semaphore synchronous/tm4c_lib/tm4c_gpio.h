...
void GPIO_InitPort(const PortName_t port);

/* Enables a set of pins on a GPIO port for digital output */
void GPIO_EnableDO(const PortName_t port, uint8_t pinMap, GPIO_Drive_t driveType, GPIO_Pull_t pullType);
...
void GPIO_PinWriteValue(const PortName_t port, uint32_t pinMap, uint8_t value);

/* Enables a set of pins on a GPIO port for digital input. */
void GPIO_EnableDI(const PortName_t port, uint8_t pinMap, GPIO_Pull_t pullType);
