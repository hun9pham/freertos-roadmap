...
void hardware_init(void){
	...
	/* Turn off all LEDs */
	LEDsOff(hw_cfgLEDsRed | hw_cfgLEDsBlue | hw_cfgLEDsGreen);
}
...
static void ConfigurationSwitchInt(void){
	...
	GPIOIntRegister(GPIO_PORTF_BASE, WakeUp_Task_ISR_Handler);
	hw_cfgGPIOF_INT_PRI_ADDRESS = 7 << 21; /* Set interrupt priority GPIOF to 7 (lowest priority) */
